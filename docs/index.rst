SLSNe
=====

**slsne** is a package designed to access the light curves and parameters of the Type-I Superluminous Supernovae (SLSNe)
sample published in `this paper <https://arxiv.org/abs/your_paper_id>`_, where we describe the methods used to create
the catalog. In this documentation, we provide a number of functions to access the catalog and work with the data.

For examples on how to access the data see the Tutorials menu.

.. toctree::
   :maxdepth: 2
   :caption: User Guide

   installation
   reference

.. toctree::
   :maxdepth: 1
   :caption: Tutorials

   tutorials/lightcurve
   tutorials/kcorrect
   tutorials/bolometric
   tutorials/parameters
   tutorials/mosfit

License & Attribution
---------------------

Copyright Sebastian Gomez and `contributors <https://github.com/gmzsebastian/SLSNe/graphs/contributors>`_.
The ``slsne`` package is free software made available under the MIT License.

If you make use of this package or its data in your work, please cite our paper
(`arXiv <https://arxiv.org/abs/your_paper_id>`_,
`ADS <https://ui.adsabs.harvard.edu/Paper>`_).
